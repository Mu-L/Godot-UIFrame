...
using UIBaseTask = System.Threading.Tasks.Task<GodotUIFrame.UIBase>;
...
public partial class UIFrame : Node
{
    ...
    /// </summary>
    public static float StuckTime { get; set; } = 0.5f;

    /// <summary>
    /// UI 场景根目录。默认按 res://ui/{layer}/{type_name}.tscn 查找。
    /// </summary>
    public static string SceneRoot { get; set; } = "res://ui";
    ...
    {
        CanvasLayer canvasLayer = new() { Name = "UiLayers" };
        CanvasLayer stuckLayer = new() { Name = "StuckLayer" };
        AddChild(canvasLayer);
        AddChild(stuckLayer);
        uiLayer = canvasLayer;
        PackedScene load = ResourceLoader.Load<PackedScene>(
            "res://addons/ui_frame/runtime/stuck/stuck_panel.tscn");
    ...
    private ControlTask LoadNode(Type type)
    {
        ...
        string scenePath =
            $"{SceneRoot.TrimEnd('/')}/{ToSnakeCase(layer.GetName())}/{ToSnakeCase(type.Name)}.tscn";
        PackedScene scene = ResourceLoader.Load<PackedScene>(scenePath);
        if (scene == null)
        {
            throw new InvalidOperationException(
                $"找不到 {type.Name} 的场景：{scenePath}。可通过 UIFrame.SceneRoot 修改 UI 根目录。");
        }

        return Task.FromResult(scene.Instantiate<Control>());
    }
